...
\begin{equation}
	\dfrac{\partial u}{\partial x}(0, y) = 0, \ u(\pi, y) = 0, \ y > 0, 
\end{equation}
\begin{equation}
	\lim\limits_{y \to 0 + 0} \int\limits_0^\pi \left[\dfrac{1}{k}\dfrac{\partial u}{\partial y}(x,y) - \dfrac{\partial u}{\partial x}(x,y) + \varphi(x) \right]^2 dx = 0, \ \varphi(x) \in L_2(0,\pi) ,\ k \in (-\infty, 0) \cup (1,  + \infty),  
\end{equation}
\begin{equation}
...
\begin{equation}
	u(x,y) = \sum\limits_{n=0}^{\infty} A_n e^{-\left(n + \dfrac12\right)y} \cos{\left[\left(n + \dfrac12\right)x\right]},
\end{equation}
\textit{где коэффициенты $A_n, \ n =0,1,2, \dots$ определяются из разложения}
\begin{equation}
	\sum\limits_{n=0}^{\infty} A_n \left(n + \dfrac12 \right) \sin{\left[\left(n +\dfrac12\right)x  - \arctg{\dfrac{1}{k}}\right]} = \dfrac{-k}{\sqrt{1+k^2}} \varphi(x).
\end{equation}

\par	
\textbf{Доказательство.} Докажем существование решения задачи $(1 - 4)$. В силу основного результата работы $[2]$ система  $\left\{\sin{\left[\left(n + 1/2\right)x - \arctg{1/k} \right]}\right\}_{n=0}^{\infty}$ образует базис Рисса в пространстве $L_2(0, \pi)$ при $k\in(-\infty, 0) \cup (1, +\infty)$. Поэтому справедливо двухстороннее неравенство Бесселя:
\begin{equation*}
...
\begin{equation*}
	M(y) = \dfrac{1}{k} \dfrac{\partial u}{\partial y} - \dfrac{\partial u}{\partial x} + \varphi(x) =
\end{equation*}
\begin{equation*}
= \sum\limits_{n=0}^{\infty}  \left[A_n \left(n + \dfrac12 \right)e^{-\left(n+\dfrac12\right)} \left( -\dfrac{1}{k} \cos{\left[\left(n+\dfrac12\right)x\right]} + \sin{\left[\left(n+\dfrac12\right)x\right]} \right) \right] + \varphi(x) = 
\end{equation*}
\begin{equation*}
	= \sum\limits_{n=0}^{\infty}  \left[ \dfrac{\sqrt{1 + k^2}}{k}A_n \left(n + \dfrac12 \right)e^{-\left(n+\dfrac12\right)} \left( \sin{\left[\left(n + \dfrac12 \right)x  - \arctg{\dfrac{1}{k}} \right]} \right) \right] + \varphi(x) = 
\end{equation*}
\begin{equation*}
	=  \dfrac{\sqrt{1 + k^2}}{k} \sum\limits_{n=0}^{\infty}  \left[A_n \left(n + \dfrac12 \right) \left(e^{-\left(n+\dfrac12\right)} - 1\right) \sin{\left[\left(n + \dfrac12 \right)x  - \arctg{\dfrac{1}{k}} \right]} \right].
\end{equation*}
\begin{equation*}
	I(y) =  \int\limits_0^\pi \left[M(y) \right]^2 dx.
\end{equation*}
...
\end{equation*}
\begin{equation*}
	I_1(y) = \dfrac{2\sqrt{1 + k^2}}{k}\int\limits_0^\pi \left[	\sum\limits_{n=0}^{m} A_n\left(n+\dfrac12\right) \left( e^{-\left(n+\dfrac12\right)y} - 1\right)\sin{\left[\left(n + \dfrac12 \right)x  - \arctg{\dfrac{1}{k}}\right]} \right]^2 dx, 
\end{equation*}
\begin{equation*}
	I_2(y) = \dfrac{2\sqrt{1 + k^2}}{k}\int\limits_0^\pi \left[	\sum\limits_{n=m+1}^{\infty} A_n\left(n+\dfrac12\right) \left( e^{-\left(n+\dfrac12\right)y} - 1\right)\sin{\left[\left(n + \dfrac12 \right)x  - \arctg{\dfrac{1}{k}}\right]} \right]^2 dx.
\end{equation*}
Зафиксируем  произвольное число $\varepsilon > 0$. В силу левой части неравенства Бесселя имеем оценку
\begin{equation*}
	I_2(y) =  \dfrac{2\sqrt{1 + k^2}}{k}\int\limits_0^\pi \left[	\sum\limits_{n=m+1}^{\infty} A_n\left(n+\dfrac12\right) \left( e^{-\left(n+\dfrac12\right)y} - 1\right) \sin{\left[\left(n + \dfrac12 \right)x  - \arctg{\dfrac{1}{k}}\right]} \right]^2 dx \leq 
\end{equation*}
\begin{equation*}
...
\begin{equation*}
	I_1(y) = \dfrac{2\sqrt{1 + k^2}}{k}\int\limits_0^\pi \left[	\sum\limits_{n=0}^{m} A_n\left(n+\dfrac12\right) \left( e^{-\left(n+\dfrac12\right)y} - 1\right)\sin{\left[\left(n + \dfrac12 \right)x  - \arctg{\dfrac{1}{k}}\right]} \right]^2 dx \leq
\end{equation*}
\begin{equation*}
...
\par
	\textbf{Теорема 3.} \textit{Пусть $u(x,y)$ - решение задачи $(1)-(4)$ и $|k| > 1$, тогда решение u представимо в виде}
\begin{equation}
	u(x,y) = Re \dfrac{1}{\pi} e^{\dfrac{ iz}{2}} \int\limits_0^\pi \dfrac{(\tg{t/2})^{\alpha}}{\cos{t/2}}  \dfrac{ (1+e^{iz})\sin{t}}{\left(1 - e^{i(z+t)} \right) \left(1 - e^{i(z-t)}\right)} \left(\dfrac{1+e^{iz}}{1 - e^{iz}}\right)^{\alpha}  F(t) dt,
	\end{equation}
где $z = x + iy, \ F(x) = \dfrac{k}{\sqrt{1+ k^2}}  \left(\int\limits_0^x \varphi(t) dt + \sum\limits_{n=0}^{\infty} A_n\right), \ \alpha = \dfrac{2\arctg{1/k}}{\pi}$. 
\par
Доказательство. Рассмотрим равенство (6).
\begin{equation*}
		\sum\limits_{n=0}^{\infty} A_n \left(n + \dfrac12 \right) \sin{\left[\left(n +\dfrac12\right)x  - \arctg{\dfrac{1}{k}}\right]} = \dfrac{-k}{\sqrt{1+k^2}} \varphi(x).
\end{equation*}
Проинтегрируем его от 0 до x.
\begin{equation*}
	-\sum\limits_{n=0}^{\infty} A_n \dfrac{k}{\sqrt{1 + k^2}} + \sum\limits_{n=0}^{\infty} A_n \cos{\left[\left(n +\dfrac12\right)x  - \arctg{\dfrac{1}{k}}\right]} = \dfrac{k}{\sqrt{1+ k^2}}\int\limits_0^x \varphi(t) dt,
\end{equation*}
 Заметим, что ряд $\sum\limits_{n=0}^{\infty} A_n \dfrac{k}{\sqrt{1 + k^2}} = u(0,0)$, поэтому
 \begin{equation*}
 	\sum\limits_{n=0}^{\infty} A_n \cos{\left[\left(n +\dfrac12\right)x  - \arctg{\dfrac{1}{k}}\right]} = F(x), 
 \end{equation*}
 где $F(x) = \dfrac{k}{\sqrt{1+ k^2}}  \left(\int\limits_0^x \varphi(t) dt + \sum\limits_{n=0}^{\infty} A_n\right)$.\newline
 Система синусов $\left\{\cos{\left[\left(n +\dfrac12\right)x - \arctg{1/k}\right]}\right\}_{n=0}^{\infty}$ образует базис в $L_2(0,\pi)$ при $k \in (-\infty, -1) \cup (0, + \infty)$. Поэтому для коэффициентов $A_n$ справедливо следующее представление [2]:
\begin{equation*}
	A_n\left(n+\dfrac12\right) = \int\limits_0^\pi h_{n+1}(t)F(t) dt, 
\end{equation*}
где
\begin{equation*}
...
и соотвественно
\begin{equation*}
	u(x,y) =  \sum\limits_{n=0}^{\infty}  \int\limits_0^\pi F(t)  h_{n+1}(t)  e^{-\left(n + \dfrac12\right)y} \cos{\left[\left(n + \dfrac12\right)x\right]} dt, 
\end{equation*}
или
\begin{equation*}
	u(x,y) = Re \ \sum\limits_{n=0}^{\infty}  \int\limits_0^\pi F(t)  h_{n+1}(t)  e^{-\left(n + \dfrac12\right)y} e^{i\left(n + \dfrac12\right)x} dt = 
\end{equation*}
\begin{equation*}
	= Re \ \sum\limits_{n=0}^{\infty}  \int\limits_0^\pi F(t)  h_{n+1}(t)  e^{i\left(n+\dfrac12\right) z}  dt = \vert m = n + 1 \vert = 
\end{equation*}
\begin{equation*}
	= Re \ \sum\limits_{m=1}^{\infty}  \int\limits_0^\pi F(t) h_{m}(t)  e^{i\left(m-\dfrac12\right) z}  dt = 
\end{equation*}
\begin{equation*}
	= Re \ e^{-\dfrac{iz}{2}}\ \sum\limits_{m=1}^{\infty}  \int\limits_0^\pi F(t)  h_{m}(t)  e^{im z}  dt .
\end{equation*}
Поменяем местами знаки интергирования и суммирования
\begin{equation*}
	u(x,y)  = Re \ e^{-\dfrac{iz}{2}}\  \int\limits_0^\pi F(t)  \sum\limits_{m=1}^{\infty}   h_{m}(t)  e^{im z}  dt
\end{equation*}
...
так как в нашем случае $\beta = -1, \ \gamma = -2\arctg{1/k}$.
Окончательно получаем формулу
\begin{equation*}
	u(x,y) = Re\ e^{\dfrac{-iz}{2}} \int\limits_0^\pi F(t) I(t,z) dt =
\end{equation*}
\begin{equation*}
	= Re\ e^{\dfrac{-iz}{2}} \int\limits_0^\pi \dfrac{2}{\pi}\dfrac{(2\cos{t/2})^\beta}{(\tg{t/2})^{\gamma/\pi}}  \dfrac{e^{iz} \sin{t}}{\left(1 - e^{i(z+t)} \right) \left(1 - e^{i(z-t)}\right)} (1 + e^{iz})^{-\gamma/\pi - \beta} (1- e^{iz})^{\gamma/\pi} F(t) dt =
\end{equation*}
\begin{equation*}
	= Re \dfrac{2}{\pi} e^{\dfrac{ iz}{2}} \int\limits_0^\pi \dfrac{(2\cos{t/2})^\beta}{(\tg{t/2})^{\gamma/\pi}}  \dfrac{ (1 + e^{iz})^{-\gamma/\pi - \beta} (1- e^{iz})^{\gamma/\pi}\sin{t}}{\left(1 - e^{i(z+t)} \right) \left(1 - e^{i(z-t)}\right)}  F(t) dt, 
\end{equation*}
т.е. представление:
\begin{equation*}
	u(x,y) = Re \dfrac{1}{\pi} e^{\dfrac{ iz}{2}} \int\limits_0^\pi \dfrac{(\tg{t/2})^{(2\arctg{1/k})/\pi}}{\cos{t/2}}  \dfrac{ (1+e^{iz})\sin{t}}{\left(1 - e^{i(z+t)} \right) \left(1 - e^{i(z-t)}\right)} \left(\dfrac{1+e^{iz}}{1 - e^{iz}}\right)^{(2\arctg{1/k}) / \pi}  F(t) dt.
\end{equation*}
Теорема доказана.
