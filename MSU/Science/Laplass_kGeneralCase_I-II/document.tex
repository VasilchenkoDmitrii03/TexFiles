...
\par	
\textbf{Доказательство.} Докажем существование решения задачи $(1 - 4)$. В силу основного результата работы $[2]$ система  $\left\{\sin{\left[\left(n + 1/2\right)x + \pi/2 - \arctg{1/k} \right]}\right\}_{n=0}^{\infty}$ образует базис Рисса в пространстве $L_2(0, \pi)$ при $k\in(-\infty, -1) \cup (0, +\infty)$. Поэтому справедливо двухстороннее неравенство Бесселя:
\begin{equation*}
...
\end{equation*}
\begin{equation*}
	=  \dfrac{\sqrt{1 + k^2}}{k} \sum\limits_{n=0}^{\infty}  \left[A_n \left(n + \dfrac12 \right) \left(e^{-\left(n+\dfrac12\right)} - 1\right) \cos{\left[\left(n + \dfrac12 \right)x  - \arctg{\dfrac{1}{k}} \right]} \right].
\end{equation*}
\begin{equation*}
...
\end{equation*}
\begin{equation*}
	I_1(y) = \dfrac{2\sqrt{1 + k^2}}{k}\int\limits_0^\pi \left[	\sum\limits_{n=0}^{m} A_n\left(n+\dfrac12\right) \left( e^{-\left(n+\dfrac12\right)y} - 1\right)\cos{\left[\left(n + \dfrac12 \right)x  - \arctg{\dfrac{1}{k}}\right]} \right]^2 dx, 
\end{equation*}
\begin{equation*}
	I_2(y) = \dfrac{2\sqrt{1 + k^2}}{k}\int\limits_0^\pi \left[	\sum\limits_{n=m+1}^{\infty} A_n\left(n+\dfrac12\right) \left( e^{-\left(n+\dfrac12\right)y} - 1\right)\cos{\left[\left(n + \dfrac12 \right)x  - \arctg{\dfrac{1}{k}}\right]} \right]^2 dx.
\end{equation*}
Зафиксируем  произвольное число $\varepsilon > 0$. В силу левой части неравенства Бесселя имеем оценку
\begin{equation*}
	I_2(y) =  \dfrac{2\sqrt{1 + k^2}}{k}\int\limits_0^\pi \left[	\sum\limits_{n=m+1}^{\infty} A_n\left(n+\dfrac12\right) \left( e^{-\left(n+\dfrac12\right)y} - 1\right) \cos{\left[\left(n + \dfrac12 \right)x  - \arctg{\dfrac{1}{k}}\right]} \right]^2 dx \leq 
\end{equation*}
\begin{equation*}
...
\begin{equation*}
	I_1(y) = \dfrac{2\sqrt{1 + k^2}}{k}\int\limits_0^\pi \left[	\sum\limits_{n=0}^{m} A_n\left(n+\dfrac12\right) \left( e^{-\left(n+\dfrac12\right)y} - 1\right)\cos{\left[\left(n + \dfrac12 \right)x  - \arctg{\dfrac{1}{k}}\right]} \right]^2 dx \leq
\end{equation*}
\begin{equation*}
