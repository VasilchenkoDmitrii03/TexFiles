...
\begin{equation}
	u(x,y) = Re \dfrac{1}{\pi} e^{\dfrac{ iz}{2}} \int\limits_0^\pi \dfrac{(\tg{t/2})^{\gamma/\pi}}{\cos{t/2}}  \dfrac{ (1+e^{iz})\sin{t}}{\left(1 - e^{i(z+t)} \right) \left(1 - e^{i(z-t)}\right)} \left(\dfrac{1+e^{iz}}{1 - e^{iz}}\right)^{\gamma / \pi}  F(t) dt,
	\end{equation}
где $z = x + iy, \ F(x) = \dfrac{k}{\sqrt{1+ k^2}}  \left(\int\limits_0^x \varphi(t) dt + \sum\limits_{n=0}^{\infty} A_n\right), \ \gamma = 2\arctg{1/k}$. 
\par
...
\end{equation*}
 ...
 Система синусов $\left\{\cos{\left[\left(n +\dfrac12\right)x - \arctg{1/k}\right]}\right\}_{n=0}^{\infty}$ образует базис в $L_2(0,\pi)$ при $k \in (-\infty, 0) \cup (1, + \infty)$. Поэтому для коэффициентов $A_n$ справедливо следующее представление [2]:
\begin{equation*}
