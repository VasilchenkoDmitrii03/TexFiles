...
где $z = x + iy$. 
\par
Доказательство. Рассмотрим равенство (6). Система синусов $\left\{\sin{\left[\left(n +1/2\right)x + \pi/4\right]}\right\}_{n=0}^{\infty}$ образует базис в $L_2(0,\pi)$. Поэтому для коэффициентов $A_n\left(n+1/2\right)$ справедливо следующее представление [2]:
\begin{equation*}
...
\end{equation*}
\begin{equation*}
	= - \mathrm{Im}\  \dfrac{2}{\pi} e^{\dfrac{-iz}{2}} \int\limits_0^\pi \dfrac{1}{2\cos{t/2} \sqrt{\tan{t/2}}}  \dfrac{e^{iz} \sin{t}}{\left(1 - e^{i(z+t)} \right) \left(1 - e^{i(z-t)}\right)} \sqrt{1 - e^{i2z}} \dfrac{\varphi(t)}{\sqrt2} dt, 
\end{equation*}
т.е. представление:
\begin{equation*}
	u_y(x,y) = - \mathrm{Im}\  \dfrac{ \sqrt{1 - e^{i2z}} }{\pi} e^{\dfrac{iz}{2}}  \int\limits_0^\pi  \dfrac{\sqrt{\sin{t}} }{\left(1 - e^{i(z+t)} \right) \left(1 - e^{i(z-t)}\right)}  \varphi(t) dt.
